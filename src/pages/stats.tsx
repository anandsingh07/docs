...
export default function StatsPage(): JSX.Element {
  ...
  const totalDisputes = consolidatedCircles.reduce((s: number, c: Circle) => s + (parseInt(c.metrics.raisedDisputesCount, 10) || 0), 0);
  const totalResolved = consolidatedCircles.reduce((s: number, c: Circle) => s + (parseInt(c.metrics.resolvedDisputesCount, 10) || 0), 0);
  ...
  const globalResolutionRate = totalDisputes > 0 ? Math.min(100, Math.round((totalResolved / totalDisputes) * 100)) : 100;
